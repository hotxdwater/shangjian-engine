...
if page == "上传与预览数据":
    ...
    st.markdown("用于上传Excel格式财务数据并快速预览各Sheet内容。")
    if source_data:
...
if page == "字段清洗与结构校验":
    ...
    st.markdown("对财务科目字段进行统一清洗，消除空格与特殊符号，便于后续处理。")
    if source_data:
...
if page == "字段筛选提取":
    ...
    st.markdown("从上传文件中提取你关心的字段，支持多列选择查看与复制。")
    if source_data:
...
if page == "BPC报表自动填报":
    ...
    st.markdown("根据字段映射自动填充资产负债表和利润表，减少手工录入，提高效率。")
